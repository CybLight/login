...
export type AuthFooterOptions = {
  ...
  showHackedLink?: boolean;
};

export function buildAuthFooter(options: AuthFooterOptions = {}): string {
  const { showLangSwitcher = true, showHackedLink = true } = options;
  ...
  const hackedBlock = showHackedLink
    ? `
...
export function initFooterLangSwitcher(): void {
