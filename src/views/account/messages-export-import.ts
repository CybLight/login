...
async function resolveLogin(): Promise<string> {
  try {
    ...
    const user = (data?.user || data?.data?.user || data) as Record<
      string,
      unknown
    >;
...
export function downloadChatsExportFile(
  exportData: ChatsExportFile,
  login: string,
): void {
  const safeLogin = login.replace(/[^\w.-]+/g, "_") || "user";
  const stamp = new Date(exportData.exportedAt || Date.now())
    .toISOString()
    .slice(0, 10);
...
export async function exportChats(api: MessagesApi): Promise<void> {
  ...
  const totalMessages = exportData.chats.reduce(
    (sum, chat) =>
      sum + (Array.isArray(chat.messages) ? chat.messages.length : 0),
    0,
  );
...
export function renderMessagesExportImportSettingsHtml(): string {
  return `
        <div class="messages-settings-section">
          ...
          <p class="messages-settings-section__hint">${t("Сохраните переписку в файл или восстановите её из резервной копии этого аккаунта (ключи шифрования сохраняются через вкладку Безопасность).")}</p>
...
export function bindMessagesExportImportHandlers(
  root: ParentNode,
  api: MessagesApi,
): void {
  const exportBtn = root.querySelector(
    "#messagesExportChatsBtn",
  ) as HTMLButtonElement | null;
  const importBtn = root.querySelector(
    "#messagesImportChatsBtn",
  ) as HTMLButtonElement | null;
  const fileInput = root.querySelector(
    "#messagesImportChatsInput",
